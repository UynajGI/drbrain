...
## 项目结构
知识抽取服务位于 `src/drbrain/extractor/` 目录下，围绕“构建阶段代理（BuildAgent）”与“LLM客户端”两条主线组织：
...
## 结论
本知识抽取服务以阶段化代理为核心，结合LLM回退链、严格的数据验证与置信度传播机制，实现了从学术论文中抽取概念、关系与引用的完整闭环。通过多源引用扩展与规则挖掘，进一步增强了知识图谱的完整性与可解释性。建议在生产环境中合理配置并发与缓存策略，并持续监控LLM指标与校验结果，以获得稳定可靠的抽取效果。

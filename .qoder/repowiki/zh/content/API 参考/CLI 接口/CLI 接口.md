...
## 简介
本文件为 DrBrain CLI 的完整命令行 API 参考文档，覆盖 ingest、query、build、analysis、export 等主要功能组，并提供命令组合使用最佳实践、错误处理与调试技巧。DrBrain CLI 基于 Typer 构建，通过统一入口注册各子命令与子应用（graph、ws），并在执行前加载配置与初始化日志。
...
## 结论
DrBrain CLI 提供了从数据摄取、知识图谱构建、检索查询、分析推理到导出运维的完整命令行能力。通过模块化设计与统一入口，用户可以灵活组合命令完成复杂任务。建议在生产环境中配合 --json 与 --dry-run 使用，并定期运行 drbrain check 与 backup 保障系统稳定与数据安全。

...
**更新** 新增多轮对话和工具调用支持，通过 `call_with_messages` 和 `acall_with_messages` 方法实现复杂的交互式会话。
...
- [llm_client.py:1-290](file://src/drbrain/extractor/llm_client.py#L1-L290)
...
- [session_agent.py:1-466](file://src/drbrain/extractor/session_agent.py#L1-L466)
...
- LLM 客户端：支持多模型回退链、JSON 响应解析、指标记录、异步/同步调用、**多轮对话和工具调用**
- 置信度队列：按置信度路由抽取项，支持共识检测与批量处理
- 会话代理（SessionAgent）：**新增** 支持多轮对话、工具调用循环、上下文压缩
...
- [llm_client.py:12-290](file://src/drbrain/extractor/llm_client.py#L12-L290)
...
**更新** 新增会话代理支持多轮对话和工具调用，通过 `call_with_messages` 和 `acall_with_messages` 实现复杂的交互式会话流程。
...
Note over Session,LLM : 多轮对话流程
Session->>LLM : "acall_with_messages(messages, tools)"
LLM-->>Session : "text, tool_calls, usage"
Session->>Session : "执行工具调用"
Session->>LLM : "再次调用含工具结果"
LLM-->>Session : "最终答案"
Note over Agent,LLM : 标准抽取流程
...
- [llm_client.py:156-270](file://src/drbrain/extractor/llm_client.py#L156-L270)
- [session_agent.py:231-295](file://src/drbrain/extractor/session_agent.py#L231-L295)
...
- 功能特性
  ...
  - **新增** 多轮对话支持：call_with_messages 和 acall_with_messages
  - **新增** 工具调用支持：支持 tools 参数和工具调用循环
...
**更新** 新增 `call_with_messages` 和 `acall_with_messages` 方法，支持：
- 直接传入预构建的消息列表，支持多轮对话历史
- 工具调用参数传递，实现多轮工具调用循环
- 返回结构化的文本、工具调用和使用量信息
...
Build --> Tools{"是否包含 tools？"}
Tools --> |是| ToolCfg["配置工具调用参数"]
Tools --> |否| Try{"调用成功？"}
ToolCfg --> Try
Try --> |是| Parse["解析 JSON 响应"]
Parse --> Extract["提取工具调用"]
Extract --> Record["记录指标"]
Record --> Done(["结束"])
Try --> |否| Next{"还有下一个模型？"}
Next --> |是| Build
...
- 设计要点
  - 支持多轮对话历史管理，自动维护消息列表
  - 工具调用循环：支持 LLM 调用 → 工具执行 → 再调用的循环
  - 上下文压缩：超过令牌预算时自动压缩历史消息
  - 会话持久化：数据库中保存完整的对话历史
- 关键功能
  - ask() 方法：处理单个问题，支持工具调用循环
  - chat() 方法：交互式聊天模式
  - 上下文管理：自动压缩和清理过期消息
- 工具调用流程
  - LLM 返回工具调用请求
...
**新增** 会话代理系统，通过 `acall_with_messages` 实现复杂的多轮对话和工具调用循环。

```mermaid
flowchart TD
User["用户提问"] --> CreateMsg["创建用户消息"]
CreateMsg --> CallLLM["acall_with_messages"]
CallLLM --> HasTools{"是否有工具调用？"}
HasTools --> |否| SaveAns["保存答案并返回"]
HasTools --> |是| ExecTools["执行工具调用"]
ExecTools --> AddToolRes["添加工具结果到消息列表"]
AddToolRes --> CallLLM2["再次调用 LLM"]
CallLLM2 --> SaveAns
SaveAns --> Compress{"是否需要压缩上下文？"}
Compress --> |是| Summarize["生成上下文摘要"]
...
**图表来源**
- [session_agent.py:231-295](file://src/drbrain/extractor/session_agent.py#L231-L295)
- [llm_client.py:219-270](file://src/drbrain/extractor/llm_client.py#L219-L270)

**章节来源**
- [session_agent.py:1-466](file://src/drbrain/extractor/session_agent.py#L1-L466)
- [test_session_agent.py:106-156](file://tests/test_session_agent.py#L106-L156)
...
- 表结构概览
  ...
  - **新增** agent_sessions、agent_messages：会话代理对话历史
- 迁移机制
...
- 组件耦合
  ...
  - **新增** SessionAgent 依赖 LLMClient 的多轮对话功能和 Database 的会话存储
  - CLI 依赖配置系统与日志初始化
- 外部依赖
  - litellm：多提供商统一接口
  - SQLite：本地持久化
...
- [queue.py:10-31](file://src/drbrain/extractor/queue.py#L10-L31)
- [session_agent.py:19-231](file://src/drbrain/extractor/session_agent.py#L19-L231)
...
- **新增** 多轮对话失败
  - 现象：acall_with_messages 返回 None 或工具调用循环中断
  - 排查：检查消息列表格式、工具定义、令牌预算、上下文压缩设置
  - 参考：[llm_client.py:156-270](file://src/drbrain/extractor/llm_client.py#L156-L270)、[session_agent.py:231-295](file://src/drbrain/extractor/session_agent.py#L231-L295)
- 输出校验失败
  - 现象：代理抛出异常或返回失败状态
...
## 结论
DrBrain 的 LLM 代理系统通过阶段化代理、回退链、置信度队列与数据库持久化，实现了稳定、可观测、可扩展的知识抽取流水线。**新增的会话代理系统**进一步增强了系统的交互能力，支持多轮对话和工具调用循环。配置系统与 CLI 降低了使用门槛；指标系统提供了成本与性能洞察。建议在生产环境中合理设置并发与阈值、启用多模型回退链，并结合指标持续优化模型选择与提示词。
...
- **新增** 异步调用 LLM（标准模式）
  - [llm_client.py:92-114](file://src/drbrain/extractor/llm_client.py#L92-L114)
- **新增** 多轮对话调用
  - [llm_client.py:156-216](file://src/drbrain/extractor/llm_client.py#L156-L216)
- **新增** 异步多轮对话调用
  - [llm_client.py:219-270](file://src/drbrain/extractor/llm_client.py#L219-L270)
- **新增** 会话代理使用示例
  - [session_agent.py:231-295](file://src/drbrain/extractor/session_agent.py#L231-L295)
...
- [config.example.yaml:1-145](file://config.example.yaml#L1-L145)
- [agent.py:73-135](file://src/drbrain/extractor/agent.py#L73-L135)
- [llm_client.py:92-270](file://src/drbrain/extractor/llm_client.py#L92-L270)
- [session_agent.py:231-295](file://src/drbrain/extractor/session_agent.py#L231-L295)
- [queue.py:10-106](file://src/drbrain/extractor/queue.py#L10-L106)
- [main.py:80-92](file://src/drbrain/cli/main.py#L80-L92)
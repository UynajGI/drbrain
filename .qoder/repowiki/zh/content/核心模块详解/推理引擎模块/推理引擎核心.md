...
## 引言
本文件面向 DrBrain 推理引擎核心模块，系统化阐述其整体架构与关键机制：推理调度（ReasonerAgent）、规则管理（图规则与路径规则）、执行监控（队列与缓存）、结果整合（假设生成、置信度传播）以及与各子系统的集成（因果链分析、双向验证、嵌入与检索）。文档同时给出任务生命周期管理、扩展性设计（插件化构建阶段、可插拔 LLM 模型链）、性能优化策略与排障建议，帮助读者快速理解并高效使用该推理引擎。

## 项目结构
推理引擎位于 src/drbrain 下，围绕“抽取器（extractor）+ 图引擎（graph）+ 存储（storage）+ 校验（validator）”组织：
- 抽取器：负责构建阶段代理（Ontology/Entities/Relations/Coref/Refine）、因果链、置信度传播、假设生成、规则挖掘、队列与缓存、检测器等。
...
A2["causal_chain.py<br/>因果链"]
A3["confidence_propagation.py<br/>置信度传播"]
A4["hypothesis.py<br/>假设生成"]
A5["rule_miner.py<br/>规则挖掘"]
A6["queue.py<br/>置信度队列"]
A7["cache.py<br/>API 缓存"]
A8["detection.py<br/>论文类型检测"]
A9["llm_client.py<br/>LLM 客户端"]
end
subgraph "图引擎"
...
- 推理调度器在图与文本检索之间穿插，驱动 LLM 进行探索与假设生成。
...
+tool_definitions() 列表
+reason(question, max_turns) 字符串
+reason_bidirectional(question, max_rounds) 字典
+_kg_validate(hypothesis) 字典
}
class GraphEngine {
+traverse(...)
+closure(...)
...
ReasonerAgent --> GraphEngine : "使用"
ReasonerAgent --> Database : "读取/写入"
```
...
## 依赖分析
- 抽取器与图引擎：因果链、置信度传播、假设生成依赖图引擎；规则挖掘依赖图与嵌入；队列依赖数据库；缓存独立。
- LLM 客户端：被抽取器与检测器广泛使用，支持同步/异步与回退链。
...
## 结论
DrBrain 推理引擎通过“抽取器 + 图引擎 + 存储 + 校验”的协同，实现了从论文到知识图谱再到可解释推理的完整闭环。ReasonerAgent 在图与文本检索之间穿插，结合双向验证与 KG 模式检测，显著提升了推理质量与可解释性。规则闭包、路径规则与嵌入增强进一步丰富了图谱语义。队列与缓存机制保障了工程化落地的稳定性与效率。配置系统与 LLM 客户端提供了灵活的扩展与运维能力。

[本节为总结，无需特定文件来源]

## 附录
- 任务生命周期管理
  - 创建：通过抽取器代理或直接调用 ReasonerAgent。

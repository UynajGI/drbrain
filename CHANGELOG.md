...
- **Unified tree RAG core** (`src/drbrain/tree/`): one shared vector store (each leaf/region vector computed once and reused by the vector leg and the builder), frozen two-stage assignment with structural reweighting (λ now actually moves membership; λ=0 is an exact identity ablation), bounded-round builder with a cost gate, `rag prepare` stages whose signature binds the resolved index model + tokenizer + embedding profile + clustering/cost params (a deployment change retires the previous contract's regions and rebuilds), a request-scoped model-driven navigator whose evidence can only come from `ReadReceipt`s, a read-only generation reader (`tree/leg.py`, `tree/reading.py`) used by the production tree leg with same-node/same-revision verification, and immutable generation publication. Protocol: `docs/unified-tree-algorithms.md`.
...
- **Canonical-first ingest**: `ingest` and `ingest-link` register the body canonically (revision + blocks + leaves) and that write is required — a failure rolls the paper back and reports it failed; per-paper `raw.md`/`tree.json` are no longer written for new papers (legacy files stay readable). `rag prepare` defaults to the unified index; `--legacy-sql` keeps the deprecated derived SQLite working copy (warning on stderr). Default prepare rejects `--paper` (use `rag index --paper`).

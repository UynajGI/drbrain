...
- **Shard merge carries the unified store** (`scripts/pipeline/merge_shards.py`): shard databases now also contribute `document_revisions`, `content_blocks` and the published leaf nodes (`tree_nodes kind='leaf'`) to the main database — shard-local regions deliberately stay behind, the FTS index follows via the `content_blocks` triggers, and the ANN/hierarchy are rebuilt once on the main database by `index build` (the per-shard `embed --tree` step therefore has a replacement path; migrating the shard ingest to write canonical content is the remaining follow-up).
...
- **Tree retrieval wiring**: the production tree leg (SQL and LlamaIndex routes) reads the published unified generation instead of the retired `tree_vectors.tree_layer='pageindex'` ANN, verifies every hit against the same node id and content revision, and fails closed when no generation exists. With no SQL corpus, a tree-only ask is served from the generation and other requested legs report `source_unavailable`.

- **Shard pipeline migration deferred**: `scripts/pipeline/shard_pipeline.sh` and
  `oa_shard_pipeline.sh` keep their legacy stage 3 (`embed --tree --db <shard> --papers <ids>`).
  `merge_shards.py`/`load_build_merge.py` only carry
  `papers/paper_ids/concepts/edges/tree_vectors/tree_summaries`, so running `index build` per shard
  would publish unified artifacts (`node_vectors`/`tree_nodes`/`content_fts`) the merge path cannot
  consume (and pay for a per-shard hierarchy). Migrating the shard pipeline waits for the merge path
  to understand the unified tables (follow-up round); `index build` serves the main corpus only.

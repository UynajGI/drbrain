...
- **Compatibility aliases (hidden)**: `query`, `hybrid`, `fsearch`, `build`, `embed`, `closure`, `rag prepare`, `rag index`, `rag health` and bare `index` stay callable with their original flags, defaults, exit codes and JSON contracts; they are hidden from the help page and print one migration line on stderr. The missing-index hint is engine aware and names the command that really prepares that engine's artifacts: `drbrain index build` for `rag_engine: sql`, `drbrain rag index` for `rag_engine: llamaindex` (the compatibility command that owns the persisted LlamaIndex generation). Project skills that still document the historical names keep working through these aliases and will be migrated to the new names in a follow-up (the `index`, `fsearch` and `paper-query` skills were updated in this change).

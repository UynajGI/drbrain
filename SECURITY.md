...
Please report security issues by emailing <yuunagi.cn@outlook.com> with:
...
- **API keys** (LLM, MinerU, Semantic Scholar) — stored in `config.local.yaml` (git-ignored) or environment variables. Never committed to version control.
...
- Review PDF sources before ingestion — MinerU processes PDFs locally or via cloud API requests
- Never share your `config.local.yaml` or environment files

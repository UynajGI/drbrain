...
|-------|---------------|------|
| `paper-ingest` | `ingest`, `fetch` | PDF download + ingest pipeline (MinerU → tree → paper record) |
...
| `kg-reason` | `reason`, `ask` | LLM agent reasoning over KG (with bidirectional validation) |
...
| Skill | CLI Command(s) | What |
|-------|---------------|------|
| `paper-query` | `query`, `fsearch` | BM25 + graph-enhanced search over concepts/arguments |
| `graph` | `graph neighbors/path/related/describe/query/traverse-from` | Graph traversal and query subcommands |
...
| `backup` | `backup` | Local tar.gz + rsync remote backup |
...
index            → index
paper-query      → query, fsearch
graph            → graph neighbors/path/related/describe/query/traverse-from
explore          → explore
fsearch          → fsearch

...
def test_make_local_id_prefers_doi() -> None:
    assert make_local_id(_rec("u1", doi="10.1000/ABC")) == canonical_paper_id(
        PaperIDs(doi="10.1000/abc")
    )
...
def test_make_local_id_normalizes_doi_url() -> None:
    assert make_local_id(_rec("u1", doi="https://doi.org/10.1000/ABC")) == canonical_paper_id(
        PaperIDs(doi="10.1000/abc")
    )
...
def test_ingest_corpus_inserts_papers() -> None:
    ...
    try:
        src = FakeSource(
            [_rec("u1", doi="10.1000/a", year=2020, abstract="abs"), _rec("u2", year=2021)]
        )
        ...
        assert prov == 2
        canonical = canonical_paper_id(PaperIDs(doi="10.1000/a"))
...
def test_ingest_corpus_dedup_by_unique_id() -> None:
    db, td = _tmp_db()
    try:
        src = FakeSource([_rec("u1", doi="10.1000/a")])
...
def test_ingest_corpus_doi_secondary_dedup() -> None:
    ...
    try:
        ...
        db.insert_paper_ids("existing_id", doi="10.1000/a")
        db.conn.commit()

        src = FakeSource([_rec("u-new", doi="10.1000/a")])
        stats = ingest_corpus(db, src, limit=10)
...
def test_ingest_corpus_links_existing_external_owner_and_keeps_following_record() -> None:
    ...
    try:
        ...
        db.insert_paper_ids("existing", openalex_id="W999999")
        db.conn.commit()
        ...
        assert db.find_corpus_source("fake", "W999999") == "existing"
        good_id = canonical_paper_id(PaperIDs(doi="10.1000/ok"))
...
def test_ingest_corpus_rolls_back_late_write_failure(monkeypatch) -> None:
    ...
    try:
        ...
        src = FakeSource([_rec("bad", doi="10.1000/late"), _rec("good", doi="10.1000/ok")])

        stats = ingest_corpus(db, src, limit=10)

        assert stats.inserted == 1
        assert len(stats.errors) == 1
        assert db.get_paper(canonical_paper_id(PaperIDs(doi="10.1000/late"))) is None
        good_id = canonical_paper_id(PaperIDs(doi="10.1000/ok"))
        assert db.get_paper(good_id) is not None
        assert db.find_corpus_source("fake", "good") == good_id
    finally:
...
def test_ingest_citations_writes_edges() -> None:
    ...
    try:
        src = FakeSource(
            [_rec("u1", doi="10.1000/a")],
            relations={
                "u1": PaperRelations(
                    unique_id="u1", references=[{"id": "10.9000/z"}, {"id": "10.9000/y"}]
                )
            },
        )
        ingest_corpus(db, src, limit=10)

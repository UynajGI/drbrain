"""Office document inspection — structured text summaries for AI agents.

Provides DOCX/PPTX/XLSX inspection without a GUI, useful for
verifying layout, content, and detecting common issues.
"""
...
# EMU per inch (used by python-pptx)
...
def inspect(path: Path, fmt: str | None = None) -> str:
    ...
    logger.info("[document] inspecting %s — fmt=%s", path.name, fmt)
    return func(path)
...
def inspect_pptx(path: Path) -> str:
    ...
    try:
        ...
        raise ImportError("python-pptx is not installed; run: pip install drbrain[office]")
    ...
    for i, slide in enumerate(prs.slides, 1):
        ...
        for shape in slide.shapes:
            ...
            if right > sw + 0.1:
                warnings.append(
                    f'  Overflow: shape right edge {right:.1f}" > slide width {sw:.1f}"'
                )
            if bottom > sh + 0.1:
                warnings.append(
                    f'  Overflow: shape bottom edge {bottom:.1f}" > slide height {sh:.1f}"'
                )
            ...
            if shape.has_text_frame:
                ...
                for para in tf.paragraphs:
                    ...
                    if text_len == 0:
                        est_height += line_height_in * 0.5
                        continue
                    chars_per_line = max(1, int(w * 72 / (font_size * 0.55)))
                    est_lines = max(1, -(-text_len // chars_per_line))
    ...
    img_count = sum(
        1 for slide in prs.slides for s in slide.shapes if s.shape_type == MSO_SHAPE_TYPE.PICTURE
    )
...
def inspect_docx(path: Path) -> str:
    ...
    try:
        ...
        raise ImportError("python-docx is not installed; run: pip install drbrain[office]")
    ...
    lines.append(
        f"Paragraphs: {para_count} | Tables: {table_count} | Images: {image_count}"
        f" | Headings: {sum(h_counts.values())} ({h_str})"
    )
...
def inspect_xlsx(path: Path) -> str:
    ...
    try:
        ...
        raise ImportError("openpyxl is not installed; run: pip install drbrain[office]")
    ...
    try:
        ...
        lines.append("--- Summary ---")
        lines.append(
            f"Worksheets: {len(wb.sheetnames)} | Total rows: ~{total_rows} | Charts: {chart_total}"
        )

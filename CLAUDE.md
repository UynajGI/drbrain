...
- **Graph-Enhanced Search** (`query` + `graph/engine.py`): `query --neighbors` uses `traverse()` for directed, relation-filtered graph expansion from BM25 results. Supports `--relation` (comma-separated edge types) and `--direction` (forward/backward/both). Returns concept nodes (not just papers) with `_via_graph`, `_source_seed`, `_distance`, `_path` fields.

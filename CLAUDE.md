...
| Loop         | `src/drbrain/loop/` (workflow.py, events.py) | Research loop 编排闭环 — LlamaIndex Workflow 12 节点（检索→抽取→gap→假设→互评→核验→沉淀→报告）+ 条件循环；4 个自由节点 agent-backed（FunctionAgent）+ 结构化 JSON 输出；settle 闭环沉淀写回 claims 表 |
